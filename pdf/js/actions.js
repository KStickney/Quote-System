$(document).ready(function(){
    ...
    /* Toggle block button Debug Block */
    $(".block_toggleDebug").click(function(){
        var bC = $(this).parents('.block').find('.content');
        if(bC.is(':visible')){
            bC.slideUp();
            $(this).find('span').removeClass('i-arrow-down-3').addClass('i-arrow-up-3');
...
function block_loading(content){
    if(content.find('.block_loading').length > 0)
        ...
        content.append('<div class="block_loading" style="width: '+(content.width()+ptc)+'px; height: '+(content.height()+ptc)+'px;"></div>');
    }
    return false;
}
