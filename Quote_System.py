...
default_sample_notes = [
    "New Warranty -- 1 year",
    "New Surplus Warranty -- 1 year",
    "Refurbished Warranty -- 1 year",
    "Surplus Never Used Warranty -- 1 year",
    "*****************************",
]
default_senders = {
    "Kyle Stickney": "kstickney@trwsupply.com",
...
completer_list = [
    ...
    "Norway",
    "Norwegian",
]  # TODO: Make completer both inline and popup
...
local_settings_file = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "data/local_settings.pickle"
)
first_time_file = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "data/first_time.txt"
)
# database_settings_file = "./data/database.pickle"
database_settings_file = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "data/database.pickle"
)


class MainWindow(
    QMainWindow
):  # TODO: make function for when click into and out of each tab + warning message box for Quote tab
    def __init__(self):
        super().__init__()
        ...
        logo = QIcon(
            os.path.join(
                ...
                "images/TRW Supply Logo.jpg",
            )
        )
        self.setWindowIcon(logo)

        self.styleSheet = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
...
class ActiveQuote(QWidget):
    def __init__(self):
        ...
        self.styleSheet = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "stylesheets/Quote Stylesheet.css",
        )
        with open(self.styleSheet, "r") as fh:
            self.setStyleSheet(fh.read())
    ...
    def UIComponents(self):
        try:
            ...
            self.add_table_item_btn.clicked.connect(
                lambda: self.addTableRow(i=None, remove_btn=True)
            )
            ...
            self.table_buttons = [
                self.add_table_item_btn,
                self.del_table_item_btn,
                self.add_note_btn,
                self.get_prices_btn,
            ]
            ...
            NOTE_COLS = 3
            NOTE_ROWS = math.ceil(
                (len(sample_notes) + 1) / NOTE_COLS
            )  # Rounds up so another col is not created
            i = 1
            j = 0
            ...
            self.additional_info_out_layout = (
                QVBoxLayout()
            )  # Make one layout for the group,label, and text box so closer together
            ...
            self.main_layout = (
                QVBoxLayout()
            )  # The main layout for the quote part of the app
    ...
    def addTableRow(self, i, remove_btn=False):
        try:
            ...
            if self.table_row_count > 20:
                ...
                msg.setText(
                    "There are 20 parts, after which number the parts will not show on the quote. "
            ...
            if action == QMessageBox.Yes or action == 1:
                if remove_btn:
                    ...
                    for row in range(1, rows):
                        try:
                            current = int(
                                self.table.itemAtPosition(row, 1).widget().text()
                            )
                            if current >= nextNum:
                                nextNum = current + 1
                        except:
                ...
                for j in range(len(table_headers)):
                    ...
                    if i == 0:  # Means Header
                        ...
                        if (
                            table_headers[j] == "Condition"
                            or table_headers[j] == "Stock"
                        ):
                            cell.setObjectName("header-combo")
                        if (
                            table_headers[j] == "Line Total"
                            or table_headers[j] == "Unit Price"
                        ):
                            cell.setMaximumWidth(200)
                        if table_headers[j] == "Part Number":
                            cell.setMaximumWidth(500)
                        if (
                            j == (len(table_headers) - 1)
                            and not self.add_note_btn.isChecked()
                        ):  # Hide Notes Column
                            cell.hide()
                    else:
                        ...
                        # elif table_headers[j] == "Line Total" or table_headers[j] == "Unit Price":
                            ...
                            False
                        ):  # FOR IF USE QDOUBLESPINBOX for line total and unit price - not used because would have to change anything that gets/set Text
                            cell = QDoubleSpinBox()
                            cell.setAlignment(QtCore.Qt.AlignCenter)
                            cell.setPrefix("$")
                        ...
                        if (
                            j == 1 or j == 4 or j == 5
                        ):  # make quantity and unity price change line total4
                ...
                if i != 0:
                    ...
                    info.setWindowStylesheet(
                        os.path.join(
                            os.path.dirname(os.path.realpath(__file__)),
                            "stylesheets/Prices Stylesheet.css",
                        )
                    )
            ...
            for i in range(1, self.table.rowCount()):
                try:
                    ...
                    if part != "":
                        ...
                        i_types, i_prices, i_stocks = getIndustrialAutomationsPrice(
                            part, driver
                        )

                        list_ = [
                            ["ApexPLC", a_types, a_prices, a_stocks],
                            ["Radwell", r_types, r_prices, r_stocks],
                        ...
                        if not yl_new_df.empty:
                            btn.addPriceSite(
                                ...
                                ["New", "Refurbished"],
                                [
                                    str(yl_new_df.iloc[0]["YL_New_Price"]),
                                    str(yl_ref_df.iloc[0]["YL_Ref_Price"]),
                                ],
                                [1, 1],
                            )
                        ...
                        if not mfr_df.empty:
                            btn.addPriceSite(
                                "Manufacturer",
                                ["New"],
                                [str(mfr_df.iloc[0]["Manufacturer_Price"])],
                                [1],
                            )
                            btn.setStyleSheet("background: rgb(30,146,244);")
    ...
    def formatShippingCharges(self):
        try:
            ...
            sendMessage(
                "Format Shipping Charges", "Please Input an Actual Number", self
            )
            self.shipping_charges.setText("")
    ...
    def calculateLineTotal(self, btn):
        try:
            ...
            if column in (2, 5):
                ...
                # get unit price
                unit = (
                    self.table.itemAtPosition(index, 5).widget().text().replace("$", "")
                )
                if unit == "":
                    unit = 0
                else:
                ...
                self.table.itemAtPosition(index, 6).widget().setText(
                    str("$" + format(quantity * unit, ".2f"))
                )
                ...
                # get unit price
                line_total = (
                    self.table.itemAtPosition(index, 6).widget().text().replace("$", "")
                )
                if line_total == "":
                    line_total = 0
                else:
                ...
                self.table.itemAtPosition(index, 5).widget().setText(
                    str("$" + format(unit_price, ".2f"))
                )
                self.table.itemAtPosition(index, 6).widget().setText(
                    str("$" + format(line_total, ".2f"))
                )
    ...
    def calculateQuoteTotal(self):
        try:
            ...
            for row in range(1, self.table.rowCount()):
                try:
                    subtotal += float(
                        self.table.itemAtPosition(row, 6)
                        .widget()
                        .text()
                        .replace("$", "")
                    )
            ...
            self.total_price.setText(
                str("$" + format(subtotal + shipping_charges, ".2f"))
            )
        except Exception as e:
            print(e)
                    ...
                    for j in range(1, self.table.columnCount()):
                        if num % 2 == 0:
                            self.table.itemAtPosition(i, j).widget().setStyleSheet(
                                f"""background: {Alternating_color}; color: {Alternating_font};"""
                            )
                        else:
                            self.table.itemAtPosition(i, j).widget().setStyleSheet("")
                except Exception as e:
    ...
    def deleteTableItemREDO(
        self,
    ):  # Completely gets rid and implements back in. Need to find a way to replace self.table, since saying self.table = QGridLayout() doesn't work to make a new instance
        ...
        try:
            ...
            for i in range(1, self.table.rowCount()):
                if i not in rows:
                    info.append([])
                    for j in range(
                        2, self.table.columnCount()
                    ):  # don't need checkbox or item number
                        try:
                            info[-1].append(
                                self.table.itemAtPosition(i, j).widget().text()
                            )
                        except:
                            try:
                                info[-1].append(
                                    self.table.itemAtPosition(i, j)
                                    .widget()
                                    .currentText()
                                )
                            except:
    ...
    def addNoteCol(
        self,
    ):  # TODO: Check see if already have note box, change j to always be same column
        try:
    ...
    def Close(self, save=False):  # save, if true, means coming from saveAndClose()
        try:
            ...
            if (
                save
            ):  # save means coming from method saveAndClose() - want to raise Exception so won't continue to close
                raise Exception
    ...
    def createPDF(self):
        try:
            ...
            html = getHTMLText(
                str(df["Part_Number"].values[0]).split(";"),
                str(df["Quantity"].values[0]).split(";"),
                str(df["Condition"].values[0]).split(";"),
                str(df["Unit_Price"].values[0]).split(";"),
                str(df["Line_Total"].values[0]).split(";"),
                str(df["Stock"].values[0]).split(";"),
                str(df["Sender"].values[0]),
                str(df["Sender_Email"].values[0]),
                str(df["Quote_Number"].values[0]),
                str(df["Customer_Email"].values[0]),
                str(df["Subject"].values[0]),
                str(df["_Date"].values[0]),
                str(df["Payment_Terms"].values[0]),
                str(df["Shipping_Method"].values[0]),
                str(df["Additional_Notes"].values[0]).split("\n"),
                df.iloc[0]["Pro_Forma"],
                df.iloc[0]["Shipping_Charges"],
                df.iloc[0]["Customer_Notes"],
            )
    ...
    def submitQuote(self):
        ...
        try:
            if (
                getQuoteLastEdited(quote_number) > self.last_time_edited
            ):  # For is is editing
                msg = QMessageBox()
                msg.setText(
                    "Another User has updated this quote since you started editing.\nWould you like to still submit?"
                )
        ...
        if action != QMessageBox.No:
            ...
            for row in range(1, self.table.rowCount()):
                try:
                    if (
                        self.table.itemAtPosition(row, 3).widget().text() != ""
                    ):  # if there is a part number, then save row
                        if self.table.itemAtPosition(row, 2).widget().text() != "":
                            quantities += (
                                self.table.itemAtPosition(row, 2).widget().text() + ";"
                            )
                        else:
                            # quantities += "None;"
                            quantities += "0;"
                        ...
                        if (
                            self.table.itemAtPosition(row, 4).widget().currentText()
                            != ""
                        ):
                            conditions += (
                                self.table.itemAtPosition(row, 4).widget().currentText()
                        ...
                        if self.table.itemAtPosition(row, 5).widget().text() != "":
                            unit_prices += (
                                self.table.itemAtPosition(row, 5).widget().text() + ";"
                            )
                        else:
                            # unit_prices += "None;"
                            unit_prices += "0;"
                        if self.table.itemAtPosition(row, 6).widget().text() != "":
                            line_totals += (
                                self.table.itemAtPosition(row, 6).widget().text() + ";"
                            )
                        else:
                            # line_totals += "None;"
                            line_totals += "0;"
                        if (
                            self.table.itemAtPosition(row, 7).widget().currentText()
                            != ""
                        ):
                            stocks += (
                                self.table.itemAtPosition(row, 7).widget().currentText()
                        ...
                        if self.table.itemAtPosition(row, 8).widget().text() != "":
                            notes += (
                                self.table.itemAtPosition(row, 8).widget().text() + ";"
                            )
                        else:
                            notes += "None;"
                except:
...
class ViewQuotes(QMainWindow):
    ...
    def UIComponents(self):
        try:
            ...
            self.tabs.tabCloseRequested.connect(
                lambda index: self.tabs.removeTab(index)
            )
    ...
    def addTabPage(self, html, quote_number):
        ...
        view.page().runJavaScript(
            "C:/Users/Patrick/Documents/Kristopher Programming/Quote-System/pdf/js/actions.js"
        )
    ...
    def printQuote(self):
        try:
            ...
            tab.page().runJavaScript(
                "document.getElementsByTagName('html')[0].innerHTML", self.makePDF
            )
    ...
    def editQuote(self):
        try:
            ...
            if main_window.tabs.widget(1).invoice_number.text() == str(
...
class Settings(QWidget):
    def __init__(self):
        ...
        self.styleSheet = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "stylesheets/Settings Stylesheet.css",
        )
        with open(self.styleSheet, "r") as fh:
            self.setStyleSheet(fh.read())
    ...
    def getLinkedFilesWid(self):
        ...
        ##SHARED SETTINGS
        self.shared_settings_lay = SettingsList(
            [shared_settings_file], "Shared Settings File", True, "Pickle", "(*.pickle)"
        )
    ...
    def getDatabaseWid(self):
        try:
            ...
            self.database_add_col_btn.clicked.connect(
                lambda: self.addCol(preferred_search)
            )
            self.database_del_col_btn = QPushButton("Delete Header")
            self.database_del_col_btn.clicked.connect(lambda: self.delCol())
            for btn in (
                self.database_submit_btn,
                self.database_add_col_btn,
                self.database_del_col_btn,
            ):
    ...
    def saveSearchBy(self):  # TODO: Make sure actually works, change _Date to __Date
        try:
            ...
            main_window.tabs.widget(0).search1.setPlaceholderText(
                "Search by " + DATABASE_SEARCHES[0].replace("_", " ")
            )
            main_window.tabs.widget(0).search2.setPlaceholderText(
                "Search by " + DATABASE_SEARCHES[1].replace("_", " ")
            )
            main_window.tabs.widget(0).search3.setPlaceholderText(
                "Search by " + DATABASE_SEARCHES[2].replace("_", " ")
            )
        except Exception as e:
        ...
        try:
            ...
            for search in DATABASE_HEADERS:  # So can choose from all database headers
                ...
                if "Date" not in search:
                    self.search_dictionary[search.replace("_", " ")] = search.replace(
                        " ", "_"
                    )
    ...
    def getUserStockCondWid(self):
        try:
            ...
            note_upup.clicked.connect(
                lambda: self.moveListItemUp(self.notes_list, True)
            )
            ...
            note_dndn.clicked.connect(
                lambda: self.moveListItemDown(self.notes_list, True)
            )
            ...
            payment_down.clicked.connect(
                lambda: self.moveListItemDown(self.payment_list)
            )
            ...
            payment_upup.clicked.connect(
                lambda: self.moveListItemUp(self.payment_list, True)
            )
            ...
            payment_dndn.clicked.connect(
                lambda: self.moveListItemDown(self.payment_list, True)
            )
    ...
    def refresh_table_maker(self, delete_tab=False):
        try:
            ...
            self.table_refresh_btn.clicked.connect(
                lambda: self.refresh_table_maker(delete_tab=True)
            )
            self.table_add_btn = QPushButton("Add Column")
            self.table_delete_btn = QPushButton("Delete Column")

            for btn in (
                self.table_submit_btn,
                self.table_refresh_btn,
                self.table_add_btn,
                self.table_delete_btn,
            ):
    ...
    def submitHeaders(self):
        ...
        try:
            ...
            for i in range(self.database_edit_layout.count()):
                database_headers.append(
                    self.database_edit_layout.itemAt(i).widget().currentText()
                )
            storeSettings()
        except Exception as e:
    ...
    def saveUsersStockConditions(self):
        try:
            ...
            sendMessage(
                "Settings Saved",
                "Quote Settings have been successfully saved",
                self,
    ...
    def save_shared_settings(self):
        try:
            ...
            sendMessage(
                "New Database Connection", file + " is the new shared file", self
            )
...
class Database(QWidget):
    def __init__(self):
        ...
        self.styleSheet = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "stylesheets/Database Stylesheet.css",
        )
        with open(self.styleSheet, "r") as fh:
            self.setStyleSheet(fh.read())
    ...
    def UIComponents(self):
        try:
            ...
            self.search1.setPlaceholderText(
                "Search by " + DATABASE_SEARCHES[0].replace("_", " ")
            )
            ...
            self.search3.setPlaceholderText(
                "Search by " + DATABASE_SEARCHES[2].replace("_", " ")
            )
    ...
    def addComboOptions(self, box):
        try:
            ...
            for search in DATABASE_HEADERS:  # So can choose from all database headers
                self.search_dictionary[
                    "Search by " + search.replace("_", " ")
                ] = search.replace(
                    " ", "_"
                )  # used to be search.lower for the key
                # self.search_by_box.addItem("Search by "+search.lower())
            box.addItems(self.search_dictionary.keys())
    ...
    def firstSearchInput(self):
        ...
        for i in range(self.listing_index, self.index):
            if (
                i > len(self.searched_df) - 1
            ):  # For if still some rows left, but under MAX_LISTINGS
                break
    ...
    def moveSearchRight(self):
        try:
            df = pandas.DataFrame()
            if self.last_click == "L" and (
                self.listing_index + self.MAX_LISTINGS
            ) < len(self.searched_df):
                self.listing_index = self.index
            self.index = self.listing_index + self.MAX_LISTINGS

            if (
                self.listing_index - 1 < len(self.searched_df) - 1
            ):  # When no more listings, don't even try to change
                for i in range(self.listing_index, self.index):
                    if (
                        i > len(self.searched_df) - 1
                    ):  # For if still some rows left, but under MAX_LISTINGS
    ...
    def onSubmitSearch(self):
        try:
            ...
            searched_df = getQuotes3(
                search1, search2, search3, search_by1, search_by2, search_by3
            )
        ...
        try:
            ...
            for i in range(len(df)):  # Rows in df
                for j in range(len(database_headers)):  # Col in grid
                    widget = ClickableLineEdit(
                        str(
                            df[(database_headers[j]).replace(" ", "_")].values[i]
                        ).replace(";", ", ")
                    )
                    ...
                    ):  # if too long for single line, move to double line qlabel
            ...
            for i in large_rows:  # rows
                for j in range(len(database_headers)):  # columns
                    if (
                        type(self.table_grid.itemAtPosition(i + 1, j).widget())
                        == QLineEdit
                        or type(self.table_grid.itemAtPosition(i + 1, j).widget())
                        == ClickableLineEdit
                    ):
                        widget = self.table_grid.itemAtPosition(i + 1, j).widget()
                        widget.setObjectName(
                            "special"
                        )  # Create special category so not confined by height rules in CSS
    ...
    def addTableHeaders(self):
        ...
        ):  # TODO: Change part column (or any with multiple lines) with QTextEdit
    ...
    def editQuote(self):
        try:
            ...
            # IF same quote already in Active Quote Tab
            if (
                main_window.tabs.widget(1).invoice_number.text() == quote_number
            ):  # str(self.searched_df["Quote_Number"].values[row]):
                main_window.tabs.setCurrentIndex(1)  # switch to active quote tab
            elif main_window.tabs.widget(1).invoice_number.text() != "":
    ...
    def continueEditQuote(self, quote_number, editing=True):
        try:
            ...
            if (
                editing
            ):  # if just copying quote, don't check see if being edited by another user
                if df.iloc[row]["isEditing"]:
                    msg = QMessageBox()
                    msg.setWindowTitle("Quote Already Open")
                    msg.setText(
                        "Quote is already being edited by another user.\nWould you like to edit anyway?"
                    )
                    msg.setIcon(QMessageBox.Critical)
                    msg.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
                    action = msg.exec()
            ...
            if action != QMessageBox.No:
                ...
                main_window.tabs.removeTab(1)
                main_window.tabs.insertTab(
                    1, ActiveQuote(), "Active Quote"
                )  # TODO: add dialog box if quote being edited
                main_window.tabs.setCurrentIndex(1)  # switch to active quote tab

                quote = main_window.tabs.widget(1)

                try:
                    quote.last_time_edited = datetime.strptime(
                        str(df.iloc[row]["Last_Edited"]), "%Y-%m-%d %H:%M:%S.%f"
                    )
                except:
                    pass
                ...
                for i in range(
                    len(part_numbers) - (INITIAL_PART_ROWS - 1)
                ):  # adds extra parts if needed
                    quote.addTableRow(i + INITIAL_PART_ROWS)

                for i in range(
                    1, quote.table.count() - 1
                ):  # TODO: make try except for each one? Or since should fill in NONE automatically, should be fine
                    try:
                        ...
                        pass
                    try:
                        quote.table.itemAtPosition(i, 3).widget().setText(
                            part_numbers[i - 1]
                        )
                    except:
                        pass
                    try:
                        quote.table.itemAtPosition(i, 4).widget().setCurrentText(
                            conditions[i - 1]
                        )
                    except:
                        pass
                    try:
                        ...
                        if unit_price == "None":
                            unit_price = "0"
                        quote.table.itemAtPosition(i, 5).widget().setText(
                    ...
                    try:
                        ...
                        if line_total == "None":
                            line_total = "0"
                        quote.table.itemAtPosition(i, 6).widget().setText(
                            "${:.2f}".format(float(line_total))
                        )
                    except:
                        pass
                    try:
                        quote.table.itemAtPosition(i, 7).widget().setCurrentText(
                            stocks[i - 1]
                        )
                    except:
                        pass
                ...
                quote.shipping_method.setCurrentText(
                    str(df["Shipping_Method"].values[row])
                )
                ...
                for i in range(main_window.tabs.widget(1).note_group_grid.count()):
                    widget = (
                        main_window.tabs.widget(1).note_group_grid.itemAt(i).widget()
                    )
    ...
    def deleteQuote(self):
        try:
            ...
            msg.setText(
                f"Are you sure you want to delete Quote {quote_number}? This action is not undoable."
            )
            msg.setStandardButtons(
                QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel
            )
    ...
    def viewQuote(self):
        try:
            ...
            column = (
                self.table_grid.itemAtPosition(
                    0, self.table_grid.getItemPosition(index)[1]
                )
                .widget()
                .text()
                .replace(" ", "_")
            ...
            main_window.tabs.widget(2).addTabPage(
                html, str(df["Quote_Number"].values[0])
            )
...
def refresh():
    ...
    for ind, tab, text in zip(
        [0, 1, 2],
        [Database(), ActiveQuote(), ViewQuotes(), Settings()],
        ["Database", "Active Quote", "View Quotes", "Settings"],
    ):
...
def setTheme(text):
    try:
        ...
        if "classic" in text:
            ...
            styleSheet = (
                "styles/dark_blue/style.qss"  # Had to change ":/" for files to "./"
            )
            folder = "dark_blue"
        elif "dark" == text:
    ...
    try:
        ...
        if local:
            ...
            with open(local_settings_file, "wb") as handle:
                pickle.dump(
                    new_local_settings, handle, protocol=pickle.HIGHEST_PROTOCOL
                )
        ...
        if shared:
            ...
            with open(shared_settings_file, "wb") as handle:
                pickle.dump(
                    new_shared_settings, handle, protocol=pickle.HIGHEST_PROTOCOL
                )
...
def storeDefaultSettings(local=True, shared=True):
    ...
    if local:
        with open(local_settings_file, "wb") as handle:
            pickle.dump(
                default_local_settings, handle, protocol=pickle.HIGHEST_PROTOCOL
            )
    ...
    if shared:
        ...
        with open(shared_settings_location, "wb") as handle:
            pickle.dump(
                default_shared_settings, handle, protocol=pickle.HIGHEST_PROTOCOL
            )
...
def loadSharedVariables(ERRORS_LIST):
    ...
    try:
        ...
        ERRORS_LIST.append(
            ["Cannot load saved payment terms. Default Settings Loaded", e]
        )
    try:
        ...
        ERRORS_LIST.append(
            ["Cannot load saved table headers. Default Settings Loaded", e]
        )
    try:
        ...
        ERRORS_LIST.append(
            ["Cannot load initial part rows. Default Settings Loaded", e]
        )
    try:
        ...
        ERRORS_LIST.append(
            ["Cannot load saved Customer Notes. Default Settings Loaded", e]
        )
    try:
    ...
    try:
        ...
        ERRORS_LIST.append(
            ["Cannot load saved stock options. Default Settings Loaded", e]
        )
    try:
        ...
        ERRORS_LIST.append(
            ["Cannot load saved part conditions. Default Settings Loaded", e]
        )
...
def sendMessage(
    title, message, parent, type=QMessageBox.Warning, btns=QMessageBox.Ok
):  # TODO: Not working if not in a Widget class
...
class FirstDialog(QDialog):
    ...
    def UIComponents(self):
        ...
        self.settings.setText(
            "//trw_nas/TRW Shared Folder/NEW Quote Database/shared_settings.pickle"
        )
    ...
    def browseSettings(self):
        settings_path = QFileDialog.getOpenFileName(
            self,
            "Open database",
            os.path.join(os.path.expanduser("~"), "Documents"),
            "PICKLE File (*.pickle)",
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            ERRORS_LIST.append(
                ["Cannot load Saved Database Headers. Default Settings Loaded", e]
            )
        try:
            ...
            ERRORS_LIST.append(
                ["Cannot load Preferred Search. Default Settings Loaded", e]
            )
        try:
        ...
        try:
            ...
            ERRORS_LIST.append(
                ["Cannot load Database Searches. Default Settings Loaded", e]
            )
        ...
        if FIRST_TIME:
            ...
            DEFAULT_USER = (
                window.user.text()
            )  # TODO: If default user not in, ask for email and make new one
