...
options.add_argument(
    str(os.path.join(os.path.dirname(os.path.realpath(__file__)), "chromedriver.exe"))
)
...
for prod in product_links:
    try:
        ...
        # Get Part Number
        product = getProduct(
            str(driver.find_element_by_class_name("product-single__title").text)
        )
...
wb.save(
    str(
        os.path.join(
            ...
            "Industrial_Automation_Prices.xlsx",
        )
    )
)
...
def add_item_ACCESS(part, price, type):
    ...
    cursor.execute(
        f"INSERT into {IA_table} (Part_Number,{type}) values ('{part}','{price}')"
    )
