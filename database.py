...
def getQuotes3(search1,search2,search3,search_by1,search_by2,search_by3):
    ...
    df = pd.read_sql("Select * from " + quote_table + f" WHERE {search_by1} LIKE '%{search1}%' AND {search_by2} LIKE '%{search2}%' AND {search_by3} LIKE '%{search3}%' ",connection)
...
def updatePartNumbers(parts_list):
    try:
        ...
        for part in parts_list:
            ...
            cursor.execute(f"SELECT Part_Number from {part_number_table} where Part_Number=?", part)
            data = cursor.fetchall()

            if not data:  # part not found in database, so add part to list
                cursor.execute(f"INSERT into {part_number_table} (Part_Number) values ('{part}')")
...
def getNewInvoiceNumber():
    ...
    invoice = str(today.month).zfill(2) + str(today.day).zfill(2) + str(today.strftime('%y')) + "-" + new
...
def submitQuoteToDatabase(quote_number,quantities,part_numbers,conditions,unit_prices,line_totals,stock,notes,subject,sender,sender_email,
                          ...
                          shipping_method,shipping_charges,pro_forma,last_edited):
...
def getYLNewPrice(part):
    df = pd.read_sql(f"Select YL_New_Price from {part_number_table} WHERE Part_Number LIKE '%{part}%'", connection)
    return df

def getYLRefPrice(part):
    df = pd.read_sql(f"Select YL_Ref_Price from {part_number_table} WHERE Part_Number LIKE '%{part}%'", connection)
    return df

def getMFRPrice(part):
    df = pd.read_sql(f"Select Manufacturer_Price from {part_number_table} WHERE Part_Number LIKE '%{part}%'", connection)
    return df
